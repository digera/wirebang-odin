...
play_click :: proc(engine: ^ma.engine) {
	if engine == nil {
		return
	}
	...
	frames := int(math.ceil_f64(f64(sr) * 0.07))
	out := make([]f32, frames * 2)
	defer delete(out)

	tick: ma.waveform
	tick_f0 := f32(2200) * (f32(0.94) + rand.float32() * f32(0.12))
	tick_f1 := max(f32(0.001), f32(900) * (f32(0.94) + rand.float32() * f32(0.12)))
	tick_cfg := ma.waveform_config_init(.f32, 1, sr, .triangle, 1, f64(tick_f0))
	if ma.waveform_init(&tick_cfg, &tick) != .SUCCESS {
		return
	}
	defer ma.waveform_uninit(&tick)
	tick_buf := make([]f32, frames)
	defer delete(tick_buf)
	read_waveform_ramp(&tick, tick_buf, sr, tick_f0, tick_f1, 0, f32(0.012), .exp)

	tickAir: ma.noise
	tickAir_cfg := ma.noise_config_init(.f32, 1, .white, i32(rand.uint32()), 1)
	if ma.noise_init(&tickAir_cfg, nil, &tickAir) != .SUCCESS {
		return
	}
	defer ma.noise_uninit(&tickAir, nil)
	tickAir_buf := make([]f32, frames)
	defer delete(tickAir_buf)
	read_noise_window(&tickAir, tickAir_buf, sr, 0, f32(0.008))

	tickAmp_peak := max(f32(0.001), f32(0.22) * (f32(0.95) + rand.float32() * f32(0.1)))
	tickAmp_buf := make([]f32, frames)
	defer delete(tickAmp_buf)
	gain_env(tick_buf, tickAmp_buf, sr, tickAmp_peak, 0, f32(0.012))

	tickHp: ma.biquad
	tickHp_f0 := f32(4200)
	if !biquad_init_rbj(&tickHp, sr, .highpass, tickHp_f0, f32(0.7)) {
		return
	}
	defer ma.biquad_uninit(&tickHp, nil)
	tickHp_buf := make([]f32, frames)
	defer delete(tickHp_buf)
	biquad_sweep(&tickHp, tickAir_buf, tickHp_buf, sr, .highpass, f32(0.7), tickHp_f0, tickHp_f0, 0)

	tickAirAmp_peak := max(f32(0.001), f32(0.07) * (f32(0.92) + rand.float32() * f32(0.16)))
	tickAirAmp_buf := make([]f32, frames)
	defer delete(tickAirAmp_buf)
	gain_env(tickHp_buf, tickAirAmp_buf, sr, tickAirAmp_peak, 0, f32(0.008))

	add_mono_to_stereo(out, tickAmp_buf)
	add_mono_to_stereo(out, tickAirAmp_buf)
